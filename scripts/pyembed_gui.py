...
Использует только стандартную библиотеку (tkinter). Вызывает pyembed через subprocess
и импортирует pyembed для списка версий и настроек.

Запуск из корня проекта: python scripts/pyembed_gui.py
"""
import os
import subprocess
import sys
...
def _pyembed_cmd(*args: str) -> list[str]:
    return [sys.executable, "-u", "-m", "pyembed.cli", *args]


def _run_cmd(tk_root, args: list[str], log_callback, done_callback):
...
def _refresh_list(listbox, root_dir: str):
    ...
    for v in list_installed(root_dir):
        ...
        try:
            ...
            if exe and path_contains(os.path.dirname(exe)):
                label += " [PATH]"
        except Exception:  # path_contains только на Windows
            pass
        listbox.insert(tk.END, label)


def _get_selected_version(listbox) -> str | None:
    """Из выбранной строки listbox извлечь номер версии (без ★ и [PATH])."""
    sel = listbox.curselection()
    if not sel:
        return None
    text = listbox.get(sel[0])
    # "3.12.0 ★ (по умолчанию) [PATH]" -> "3.12.0"
...
def main_gui() -> None:
    ...
    # Основной контейнер: слева список, справа кнопки и лог
    ...
    # Правая колонка — путь, статус, кнопки, лог
    ...
    # Кнопки действий по группам
    all_buttons: list[ttk.Button] = []
    selection_buttons: list[ttk.Button] = []

    def add_btn(parent: ttk.Frame, text: str, cmd, need_selection: bool = False) -> ttk.Button:
    ...
    # Группы кнопок (вертикально, чтобы окно не растягивалось вширь)
    ...
    # Лог вывода команд (в правой колонке)
